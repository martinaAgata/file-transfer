...
class StopAndWait:
        ...
        for _ in range(0, 5):
            try:
                ...
                while lastRcvBit == message.bit:
                    logging.debug(f"Duplicated package with bit = {message.bit} and {message.data[:15]}")
    ...
    def send_file(self, file, address, timeout):
        try:
            ...
            while data:
                ...
                message = self.receive(address, lastSentMsg=data, lastSentBit=self.bit, timeout=timeout)
                self.alternateBit()

                if message.type != ACK:
                    if message.type == FIN:
                        logging.info(f"{FIN} messsage received from {address}.")
                        self.transferMethod.sendMessage(self.bit, FIN_ACK.encode(), address)
            ...
            _ = self.receive(address, lastSentMsg=FIN.encode(), lastSentBit=self.bit, timeout=timeout)
    ...
    def recv_file(self, file, address, lastSentMsg=None, lastRcvBit=None):
        ...
        while message.type == DATA:
            ...
            self.transferMethod.sendMessage(message.bit, raw_message_to_send, message.clientAddress)
